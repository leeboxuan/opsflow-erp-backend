...
Core services include `ops-jobs.service.ts` and `driver-jobs.service.ts`. Dispatch is `src/transport/dispatch/dispatch.service.ts`. Job helpers and import DTOs live in `src/transport/jobs/` (imported by ops services and Finance; `/jobs/import/*` routes still on `OpsJobsController`). Trip helpers live in `src/transport/trips/`. Prisma `Job` and `Trip` (where `jobId` is set) are **transport-only**.
...
2. Move leaf modules with few dependencies (e.g. fleet tracking; dispatch moved to `src/transport/dispatch`; job helpers moved to `src/transport/jobs`; trip helpers moved to `src/transport/trips`)
